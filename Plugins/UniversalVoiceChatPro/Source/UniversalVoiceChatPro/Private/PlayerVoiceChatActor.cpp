...
void APlayerVoiceChatActor::BeginPlay() {
	...
	if (GetWorld()->GetNetMode() == NM_ListenServer || GetWorld()->GetNetMode() == NM_DedicatedServer) {
		...
		if (GetWorld()->GetNetMode() == NM_ListenServer) {
			...
			if (OwnedLocally) {
				UE_LOG(LogTemp, Warning, TEXT("APlayerVoiceChatActor OnRep_Owner OwnedLocally server %d"), GetWorld()->GetNetMode() == NM_ListenServer || GetWorld()->GetNetMode() == NM_DedicatedServer);
				myPlayerVoiceActor = this;
			}
			else {
				UE_LOG(LogTemp, Warning, TEXT("APlayerVoiceChatActor OnRep_Owner not OwnedLocally server %d"), GetWorld()->GetNetMode() == NM_ListenServer || GetWorld()->GetNetMode() == NM_DedicatedServer);
			}
		}
...
void APlayerVoiceChatActor::Tick(float DeltaTime) {
	...
	if (GetWorld() != NULL && (GetWorld()->GetNetMode() == NM_ListenServer || GetWorld()->GetNetMode() == NM_DedicatedServer)) {
